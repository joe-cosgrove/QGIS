...
class FeatureSourceTestCase(object):
    ...
    def xtestGetFeatures(self, source=None, extra_features=[], skip_features=[], changed_attributes={}, changed_geometries={}):
    ...
    def xassert_query(self, source, expression, expected):
    ...
    def xrunGetFeatureTests(self, source):
    ...
    def xtestGetFeaturesUncompiled(self):
        self.compiled = False
        try:
        ...
        if hasattr(self, 'poly_source'):
            self.runPolyGetFeatureTests(self.poly_source)

    def xrunOrderByTests(self):
    ...
    def testOpenIteratorAfterSourceRemoval(self):
        ...
        links between the iterators and the layer's data source
        """
    ...
    def xtestGetFeaturesFidTests(self):
    ...
    def xtestGetFeaturesFidsTests(self):
        fids = [f.id() for f in self.source.getFeatures()]
    ...
    def xtestGetFeaturesFilterRectTests(self):
    ...
    def xtestRectAndExpression(self):
        extent = QgsRectangle(-70, 67, -60, 80)
    ...
    def xtestGetFeaturesLimit(self):
    ...
    def xtestGetFeaturesSubsetAttributes(self):
    ...
    def xtestGetFeaturesSubsetAttributes2(self):
            ...
            for f in self.source.getFeatures(QgsFeatureRequest().setSubsetOfAttributes([field_to_fetch], self.source.fields())):
                ...
                for other_field in [field.name() for field in self.source.fields() if field.name().lower() != field_to_fetch]:
                    if other_field == 'pk' or other_field == 'PK':
                        # skip checking the primary key field, as it may be validly fetched by sources to use as feature id
                        continue
    ...
    def xtestGetFeaturesNoGeometry(self):
    ...
    def xtestGetFeaturesWithGeometry(self):
